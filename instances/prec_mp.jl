        ...
        for col in 1:cols
            ...
            if slot != 0
                ...
                if row-1 > 0
                    ...
                    # (if last column OR next column/diagonal right-up has zero) AND it's not the first column AND the cargo to be loaded is not zero
                    ...
                    # if not last column AND next column/diagonal right-up has not zero
                ...
                if row+1 <= rows
                    ...
                    # (if first column OR previous column/diagonal left-down has zero) AND it's not the last column AND the cargo to be unloaded is not zero
                    ...
                    # if not first column AND previous column/diagonal left-down has not zero
