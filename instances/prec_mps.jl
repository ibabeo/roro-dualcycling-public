        ...
        for col in 1:cols
            ...
            if slot != 0
                ...
                if row-1 > 0
                    ...
                    # if not last column AND cargo to be loaded is not zero
                    if col != cols && layout[col+1][row-1] != 0
                        slot_fs = layout[col+1][row-1]
                        push!(loading, ["slot"*string(slot_fs),"slot"*string(slot)])
                    end
                    # if not first column AND cargo to be loaded is not zero
                    if col != 1 && layout[col-1][row-1] != 0
                ...
                if row+1 <= rows
                    ...
                    # if not last column AND cargo to be unloaded is not zero
                    if col != cols && layout[col+1][row+1] != 0
                        slot_fs = layout[col+1][row+1]
                        push!(unloading, ["slot"*string(slot_fs),"slot"*string(slot)])
                    end
                    # if not first column AND cargo to be unloaded is not zero
                    if col != 1 && layout[col-1][row+1] != 0
                        slot_fp = layout[col-1][row+1]
                        push!(unloading, ["slot"*string(slot_fp),"slot"*string(slot)])
                    end
                end
            end
        end
